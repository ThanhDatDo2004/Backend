...
function buildWhere(filters) {
    ...
    if (filters.search) {
        ...
        conditions.push(`(LOWER(f.FieldName) LIKE ? OR LOWER(f.Address) LIKE ? OR LOWER(s.ShopName) LIKE ?)`);
...
const fieldModel = {
    ...
    async getImagesByCodes(fieldCode, imageCodes) {
    ...
    `;
        return await query_1.default.execQueryList(query, [fieldCode, imageCodes]);
    },
    async deleteImages(fieldCode, imageCodes) {
        if (!imageCodes.length)
            return 0;
        const query = `
      ...
      WHERE FieldCode = ? AND ImageCode IN (?)
    `;
        const result = await query_1.default.execQuery(query, [fieldCode, imageCodes]);
    ...
    async countByStatus(filters) {
      ...
      GROUP BY f.Status
    `;
        return (await query_1.default.execQueryList(query, params));
    },
    async countByShopApproval(filters) {
        const { clause, params } = buildWhere(filters);
        const query = `
    ...
    `;
        const rows = await query_1.default.execQueryList(query, params);
        return rows.map((row) => row.sport_type ?? "").filter(Boolean);
    },
    async listImages(fieldCodes) {
    ...
    `;
        return await query_1.default.execQueryList(query, [fieldCodes]);
    },
    async listSlots(fieldCode, playDate, quantityId) {
    ...
    async listBookingSlots(fieldCode, playDate) {
        const query = `
      SELECT
    ...
    async hasFutureBookings(fieldCode) {
        const query = `
    ...
    async deleteAllImagesForField(fieldCode) {
    ...
    async listAllImagesForField(fieldCode) {
        const query = `
      SELECT
        ImageCode AS image_code,
        FieldCode AS field_code,
      ...
      ORDER BY SortOrder, ImageCode
    `;
        return await query_1.default.execQueryList(query, [fieldCode]);
    },
    async hardDeleteField(fieldCode) {
    ...
    async deleteAllPricingForField(fieldCode) {
    ...
    async hasAnyBookings(fieldCode) {
    ...
    async deleteAllBookingsForField(fieldCode) {
    ...
    async softDeleteField(fieldCode) {
    ...
    async findShopByCode(shopCode) {
      ...
      LIMIT 1
    `;
        const rows = await query_1.default.execQueryList(query, [shopCode]);
        return (rows[0] ?? null);
    },
    async createImage(fieldCode, imageUrl, sortOrder) {
        return await query_1.default.execTransaction("fieldModel.createImage", async (conn) => {
    ...
    async updateField(fieldId, dataToUpdate) {
        const fields = [];
        const params = [];
    ...
    async insertFieldImage(conn, params) {
        const [result] = await conn.query(`
