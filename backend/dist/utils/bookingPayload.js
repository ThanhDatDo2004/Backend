...
exports.normalizeConfirmBookingRequest = void 0;
...
const normalizeConfirmBookingRequest = (body) => {
    const safeBody = body ?? {};
    const slots = normalizeSlots(safeBody.slots);
    const paymentMethod = asString(safeBody.paymentMethod) ?? asString(safeBody.payment_method);
    const totalPrice = asNumber(safeBody.totalPrice) ?? asNumber(safeBody.total_price);
    const notes = asString(safeBody.notes);
    const createdBy = asNumber(safeBody.createdBy) ?? asNumber(safeBody.created_by);
    const quantityId = asNumber(safeBody.quantityId) ?? asNumber(safeBody.quantity_id);
    const promotionCodeRaw = asString(safeBody.promotionCode) ?? asString(safeBody.promotion_code);
    const promotionCode = promotionCodeRaw?.toUpperCase();
    const customer = normalizeCustomer(safeBody.customer);
    const isLoggedInCustomer = typeof safeBody.isLoggedInCustomer === "boolean"
...
exports.normalizeConfirmBookingRequest = normalizeConfirmBookingRequest;
